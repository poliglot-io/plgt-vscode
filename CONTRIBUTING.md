# Contributing to plgt-vscode

Thanks for your interest in contributing.

## Development

Requires Node.js 22+.

```bash
npm install
npm run watch              # esbuild --watch
npm run typecheck          # tsc --noEmit
npm run lint
npm run package            # build a local .vsix
```

To install the locally built extension:

```bash
npm run install:local
```

## Reporting bugs

Open an issue with the **Bug report** template. Include the minimal reproduction, your VS Code version, `plgt` CLI version, and what you expected vs. what happened.

## Proposing changes

For anything beyond a small fix:

1. Open an issue first with the **Feature request** template so we can agree on scope.
2. Fork, branch from `main`, and make your change.
3. Run `npm run lint` and `npm run typecheck` locally — CI runs the same.
4. Open a PR against `main`. Fill out the PR template.

## Coding standards

- TypeScript strict mode.
- ESLint + Prettier defaults are enforced in CI (`npm run lint`).
- Keep the extension surface small: language registration, LSP client setup, and configuration. Anything semantic belongs in the CLI's `plgt lsp` server.

## Releasing

Releases publish to the VS Code Marketplace under publisher `poliglot`. Maintainers cut a release by tagging `vX.Y.Z`; the publish workflow handles marketplace authentication. Do not publish from a local machine.

## Code of Conduct

This project follows the [Contributor Covenant](CODE_OF_CONDUCT.md). By participating you agree to abide by its terms.

## Contributor License Agreement

Before your first PR can be merged you'll need to sign the Poliglot CLA:

- Individual: <https://poliglot.io/cla/individual>
- Overview: <https://poliglot.io/cla>

The CLA is a one-time sign — it covers all current and future Poliglot OSS repos. Our CLA bot will leave a comment on your first PR with the signing link.

## License

By contributing you agree that your contributions will be licensed under the Apache License 2.0.

